...
- **Service-worker cache version:** `v5.220` (constant `CACHE_VERSION` in `sw.js` — bumped on every deploy; cache versions are per-deploy, not per-marketing-version)
...
| `simulated-leaderboard.js` | **v3 Phase 1s — client-side ONLY.** Injects ~9 simulated hunters into the Steps leaderboard `top` array at render time so an empty/near-empty board still feels alive. Pure function (`window.SimulatedLeaderboard.merge`) with a single kill-switch constant `SIMULATE_USERS` at the top of the file. NEVER sent to the backend, NEVER persisted to localStorage. Determinism via mulberry32 seeded off a hash of the device-local date — same date = same fakes across reloads; next day = different cast + numbers. Wrapped in `_lbMaybeSimulate` inside `app.js` (called at both `openLeaderboardRanking` render points, after `lbCacheWrite` so the cache stays pure-backend). Only fires for the `step_total` metric. |
...
The current state is `styles.css?v=258`, `app.js?v=338`, `auth.js?v=7`, `simulated-leaderboard.js?v=1`, `sw.js v5.220`, `APP_VERSION = '2.2.0'` (in BOTH `app.js` and `codemagic.yaml`), `HEALTHKIT_AUTH_VERSION = 2`. (Re-check from the files; they drift quickly.)
