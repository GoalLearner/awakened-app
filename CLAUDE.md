...
- **Active duel hero** (`#duels-hero`): driven by `renderActiveDuelHero(active)` inside `renderDuelsSection`. Empty state = dashed purple border + crossed-daggers crest + "Find Hunter" button (scrolls to + focuses `#social-friend-input`). Active state = gold-glow border + Live pill + countdown chip + rival avatar + DURATION/STAKE/REWARD strip + "View Duel" gold button. **No scores rendered.** When multiple active duels exist, the hero shows the most-recently-started one (sort by `starts_at` desc).
...
- **No fake scoring anywhere.** The active hero, the active duel cards, and the duel detail overlay's score field all show a single italic "Scoring activates in the next duel pass." line. The detail overlay's `_ddoPopulate` no longer injects `You: X · Opponent: Y` even when the backend returns zeros.
- **Challenge button** uses `window.confirm("Challenge {alias} to a 3-day Discipline Duel? Stake: 25 souls. Reward: 40 souls.")`. The 25-soul stake is metadata only — no local souls deduction.
...
- **Reintroducing fake duel scores in the Active Duel hero or detail overlay.** Until server-side scoring ships, every score-rendering surface (the active hero card, the active duel card on the Duels tab, the duel detail overlay's SCORE field, `_ddoPopulate`) must show the single italic line `Scoring activates in the next duel pass.` — NEVER `You: X · Opponent: Y`, NEVER `7 vs 5`, NEVER zeroed numbers, even when the backend returns 0/0 in `challenger_score`/`opponent_score`. The columns exist on the `duels` table for the future scoring pass; they are NOT user-facing yet.
