...
- **Service-worker cache version:** `v5.216` (constant `CACHE_VERSION` in `sw.js` — bumped on every deploy; cache versions are per-deploy, not per-marketing-version)
...
The current state is `styles.css?v=256`, `app.js?v=336`, `auth.js?v=7`, `sw.js v5.216`, `APP_VERSION = '2.2.0'` (in BOTH `app.js` and `codemagic.yaml`), `HEALTHKIT_AUTH_VERSION = 2`. (Re-check from the files; they drift quickly.)
