...
- **Current marketing version:** `2.2.0` (constant `APP_VERSION` in `app.js` AND `codemagic.yaml`). v2.1.0 (build 35) was submitted for review May 13 1:39 AM PST then the train locked mid-development; the v2.2.0 train opened cleanly on commit `e744a08` and shipped to TestFlight first as **build 58** on May 14. Subsequent builds layered in additional Phase work — notably **build 63** (Phase 1u Strength training auto-verify) and **build 65** (Phase 1v D-rank dungeon: 3 new bosses + 15 new drop cards + production item artwork, smoke-tested clean and queued for App Store review). Cloud Sync v1 (Phase 1w) landed on commit `bdaac5b` post-build-65 and rides the next Codemagic train. Coverage on top of v2.1.0:
...
- **Service-worker cache version:** `v5.232` (constant `CACHE_VERSION` in `sw.js` — bumped on every deploy; cache versions are per-deploy, not per-marketing-version)
...
| `simulated-leaderboard.js` | **v3 Phase 1s (rev v3 = weekly-stable) — client-side ONLY.** Injects ~9 simulated hunters into ALL THREE leaderboard `top` arrays (step_total / sleep_streak / bedtime_streak) at render time so empty/near-empty boards still feel alive. Pure function (`window.SimulatedLeaderboard.merge(realTop, alias, value, dateKey, metric)`) with a single kill-switch constant `SIMULATE_USERS` at the top of the file. NEVER sent to the backend, NEVER persisted to localStorage. **Weekly seeding (rev v3):** simulated users are seeded by `weekStartKey + '\|' + metric`, NOT daily `dateKey`. `getWeekStartKey(dateKey)` returns the most recent Sunday (device-local). Two RNG streams per week per metric — `'\|roster'` picks the 9-name subset, `'\|values'` picks per-fake factors. Same week = same roster, same per-fake factor across all 7 days. New week = full re-roll. **Step totals accumulate deterministically through the week:** each fake's displayed value = `userAnchor × factor` where `factor` is week-stable (band 1.05–1.30 for above-user fakes, 0.40–0.90 for below). As the real user's anchor (their 7-day total) grows day-over-day, every fake's display grows proportionally — guaranteed monotonic non-decreasing per fake within the week. No accumulation curves / profiles (the initial implementation with per-fake profiles produced ~6% mid-week dips because `display = projectedFinal × factor × profile[dow]` is non-monotonic when `projectedFinal = anchor × 7/(dow+1)`; the simpler `display = anchor × factor` is the structural fix). **Sleep / bedtime streaks stable within the week:** small-integer values rolled once at week start, repeated every render Mon–Sat. New week = new streak distribution. The 3 boards rotate through DIFFERENT subsets of the 25-name pool. Wrapped in `_lbMaybeSimulate` inside `app.js` (called at both `openLeaderboardRanking` render points, after `lbCacheWrite` so the cache stays pure-backend). |
...
The current state is `styles.css?v=259`, `app.js?v=349`, `auth.js?v=8`, `simulated-leaderboard.js?v=3`, `sw.js v5.233`, `APP_VERSION = '2.2.0'` (in BOTH `app.js` and `codemagic.yaml`), `HEALTHKIT_AUTH_VERSION = 2`. (Re-check from the files; they drift quickly.)
