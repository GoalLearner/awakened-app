...
type Row = { challenger_user_id: string; partner_user_id: string; boss_id: string; resolved_at: string };
...
describe('GET /v1/coop-boss/pacts', () => {
  ...
  it('queries only WINS the caller participated in', async () => {
    const log: string[] = [];
    ...
    expect(sql).toMatch(/challenger_user_id = \? OR partner_user_id = \?/);
  });
});
