      ...
      bind: (...args: unknown[]) => {
        ...
        return {
          ...
          first: async () => {
            ...
            return null; // FOUNDER_FIRST_N='0' short-circuits the rank query anyway
          },
          run: async () => {
...
function makeEnv(db: D1Database, rlSuccess = true): Env {
  return {
    DB: db,
    FOUNDER_FIRST_N: '0', // disable the first-N promo so founder is driven purely by owned rows
...
describe('handleEntitlementsReconcile', () => {
  it('grants a purchase RevenueCat knows about but the DB is missing', async () => {
    ...
    const body = await res.json() as { ok: boolean; reconciled: number; skins: string[]; founder: boolean };
  ...
  it('maps the Founder product to the reserved entitlement + founder flag', async () => {
    stubRc({ 'com.goallearner.awakened.founders_lifetime': [{ store_transaction_id: 'txn-f' }] });
    const db = makeDb([]);
    const res = await handleEntitlementsReconcile(req(), makeEnv(db), SESSION);
    const body = await res.json() as { reconciled: number; skins: string[]; founder: boolean };
    expect(body.reconciled).toBe(1);
    expect(body.founder).toBe(true);
    expect(body.skins).toEqual([]); // 'founder' is filtered out of skins
  });
  ...
  it('refuses to grant a transaction already claimed by another user (anti-farming)', async () => {
    ...
    // re-grant it (buy-once-farm-onto-N-accounts exploit, incl. the $49.99 Founder).
