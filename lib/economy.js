...
(function (global) {
  ...
  // (3) W485 — DIFFICULTY tilt. The compound is ~90% of rank XP and was difficulty-BLIND, so a
  // routine of trivial habits reached S+ as fast as a brutal one ("rank rewards attendance, not
  // effort"). This scales the custom compound by the routine's AVERAGE habit difficulty (pts:
  // easy 1 / medium 3 / hard 5 / legendary 10), REBALANCED around medium (3 -> 1.0x, unchanged)
  // so the tuned rank pace stays put (no inflation): an all-easy routine earns ~0.7x, all-hard
  // ~1.3x, all-legendary ~1.5x (clamped). HEADROOM NOTE: no shipping LIBRARY habit is 'legendary'
  // yet and custom-CREATED habits are pinned to medium, so the OPERATIVE band TODAY is ~0.7x
  // (easy-heavy library routine) to ~1.3x (all-hard library); an all-custom-created routine sits
  // at 1.0x. The 1.5x ceiling is forward-looking (if legendary habits or per-habit difficulty
  // selection ship). Linear in avgPts away from the baseline, clamped to [MIN, MAX]; monotonic
  // non-decreasing in avgPts over its domain.
  // CUSTOM-ONLY (where the user CHOOSES difficulty) — preset packs are curated-fixed and per-habit
  // XP already rewards difficulty for everyone. Invalid/zero avgPts -> 1.0 (neutral, no tilt).
  var CUSTOM_COMPOUND_DIFF_BASELINE = 3;    // medium pts -> 1.0x pivot
  var CUSTOM_COMPOUND_DIFF_SLOPE    = 0.15; // factor change per difficulty-pt away from baseline
  var CUSTOM_COMPOUND_DIFF_MIN      = 0.7;  // floor (all-easy)
  var CUSTOM_COMPOUND_DIFF_MAX      = 1.5;  // ceiling (hard / legendary)
  function customCompoundDifficultyFactor(avgPts) {
    if (!(avgPts > 0)) return 1;
    var f = 1 + CUSTOM_COMPOUND_DIFF_SLOPE * (avgPts - CUSTOM_COMPOUND_DIFF_BASELINE);
    return Math.max(CUSTOM_COMPOUND_DIFF_MIN, Math.min(CUSTOM_COMPOUND_DIFF_MAX, f));
  }
  ...
  var API = {
    ...
    customCompoundDifficultyFactor: customCompoundDifficultyFactor,
