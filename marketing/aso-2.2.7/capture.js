// capture.js — render index.html and save each App Store shot as a real
// 1320×2868 PNG. Run:  node marketing/aso-2.2.7/capture.js
...
const FILE_URL = 'file:///' + path.join(DIR, 'index.html').replace(/\\/g, '/');
...
(async () => {
  ...
  const ctx = await browser.newContext({ deviceScaleFactor: 1, viewport: { width: 1500, height: 1000 } });
  const page = await ctx.newPage();
  await page.goto(FILE_URL, { waitUntil: 'load', timeout: 60000 });

  // in-browser Babel + React render is async; wait for all 5 nodes + every image
  ...
  for (let i = 0; i < shots.length; i++) {
    ...
    const file = path.join(OUT, IDS[i] + '.png');
